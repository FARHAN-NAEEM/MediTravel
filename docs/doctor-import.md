...
The 1,867 records were imported into the local `meditravel` database only, after applying the `source_url` migration. This does **not** update production. Once the code and CSV are on the server, run `php artisan migrate --force`, preview `php artisan doctors:import database/rosters/apollo-doctors-2026-09-17.csv`, then run the same command with `--commit` after checking the preview. The deployment workflow runs migrations but does not import this CSV automatically; a code push alone will not populate production doctors.
