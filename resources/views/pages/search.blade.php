                ...
                <div class="mt-5 grid gap-4 md:grid-cols-3">
                    @foreach ($treatments as $treatment)
                        <a href="{{ route('treatments.show', $treatment) }}" class="border border-slate-200 bg-white p-5 shadow-sm transition hover:border-tealTrust/40 hover:shadow-md">
                            <h3 class="text-lg font-bold text-navyDeep">{{ $treatment->localized('name') }}</h3>
                            <p class="mt-1 text-sm text-slate-600">{{ $treatment->department?->localized('name') }}</p>
                            <p class="mt-3 line-clamp-2 text-sm leading-6 text-slate-600">{{ $treatment->localized('description') }}</p>
                        </a>
                    @endforeach
                </div>
            </div>
